...
class Server:
    ...
    def bind(self):
        try:
            ...
            self.print_log(
                "An error occured during connecting to server: " + str(error)
            )
    ...
    def listen(self):
        self.server.listen()
        self.print_log(
            f"Server is listening on IP = {SERVER_IP} at PORT = {SERVER_PORT}"
        )
        ...
        while self.is_running:
            ...
            try:
                ...
                self.send_data(
                    list(self.players.values()), "!SET_PLAYER_COUNT", len(self.players)
                )
                self.send_data(player, "!UPDATE_ROOM", player)

                # starting a new thread
                thread = threading.Thread(
                    target=self.handle_client, args=(client_socket, player)
                )
                thread.start()
    ...
    def send_data(self, player_list, command, value=None, exceptions=None):
        ...
        player_list = [
            player for player in player_list if player not in (exceptions or [])
        ]
            ...
            while connected:
                ...
                if data:
                    ...
                    if command == "!SET_PLAYER":
                        ...
                        self.print_log(
                            f"{player.name} ({player.id}) is entered to lobby."
                        )
                        ...
                        if (
                            len(self.room_list) > 0
                            and room_id in self.room_list.keys()
                            and self.room_list[room_id].size
                            > len(self.room_list[room_id])
                        ):
                            player.join_room(self.room_list[room_id], False)
                            self.print_log(
                                f"{player.name} ({player.id}) is joined a room {room_id}."
                            )
                        ...
                        self.send_data(player, "!UPDATE_ROOM", player)
                        self.print_log(
                            f"{player.name} ({player.id}) is created a room {self.room_id}."
                        )
                        ...
                        thread = threading.Thread(
                            target=self.handle_room, args=(player.room,)
                        )
                        thread.start()
