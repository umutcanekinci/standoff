...
class Network:
    ...
    def connect(self, ip, port=PORT) -> int:
        try:
            ...
            self.status += (
                "\n"
                + "[CLIENT] => Connected to Server with IP: "
                + self.server
                + " and Port: "
                + str(port)
            )
            ...
            self.status += (
                "\n"
                + "Failed to connect to Server with IP: "
                + self.server
                + " and Port: "
                + str(port)
                + " => "
            ...
            threading.Thread(
                target=self.__recieve_from_server, args=(self.socket,)
            ).start()
            return 1
