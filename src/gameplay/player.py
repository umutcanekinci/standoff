...
class Player(Entity):
	...
	def __init__(self, id, name, name_color, character, position, size, game) -> None:
		...
		super().__init__(id, name, name_color, position, size, game.assets.image_path(f"char_{character}_gun"), PLAYER_MAX_HP, PLAYER_MAX_HP)
	...
	def rotate_to_mouse(self):
		...
		self.angle = (Vec(self.game.mouse_position) - Vec(self.game.camera.apply(self.rect).center)).angle_to(Vec(1,0)) # angle between difference vector and x axis
...
class Players(list):
	...
	def add_player(self, player_info, name_color) -> Player:
		...
		player = Player(player_info.id, player_info.name, name_color, player_info.character_name, self.game.map.spawn_points[player_info.base_number], player_info.size, self.game)
		self.append(player)
		return player

	def get_player_with_id(self, id: int) -> Player | None:

		return next((player for player in self if player.id == id), None)
