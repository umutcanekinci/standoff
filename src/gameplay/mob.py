...
class Mob(Entity):
	...
	def rotate_to_target(self):
		...
		self.angle = (Vec(self.target) - Vec(self.rect.center)).angle_to(Vec(1,0)) # angle between difference vector and x axis
	...
	def update(self):
		...
		if now - self.last_attack > 1000:
			...
			for player in [p for p in self.game.players if self.hit_rect.colliderect(p.rect)]:
...
class Mobs(list):
	...
	def add_mob(self, mob_info, character='zombie') -> None:
		...
		self.append(Mob(mob_info.id, 'Mob ' + str(mob_info.id), mob_info.position, mob_info.size, mob_info.target_base, character, self.game))
